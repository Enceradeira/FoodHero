...
class ConversationHasNoInternetConnectionTests: ConversationTestsBase {
    ...
    func test_NetworkError_ShouldTriggerFHHasNetworkErrorRepeatably() {
        ...
        assertLastStatementIs("FH:HasNetworkError", state: "networkError")

        sendInput(NetworkError())
        assertSecondLastStatementIs("FH:HasNetworkError", state: "networkError")
        assertLastStatementIs("FH:HasNetworkError", state: "networkError")
    }
    ...
    func test_UTryAgain_ShouldTriggerFHToRepeastItsLastQuestion() {
        ...
        assertLastStatementIs("FH:BeforeRepeatingUtteranceAfterError", state: "askForFoodPreference")
        assertLastStatementIs("FH:OpeningQuestion", state: "askForFoodPreference")
    }
