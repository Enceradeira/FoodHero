...
public class TalkerEngineBasicTests: TalkerEngineTests {
    ...
    func test_talk_shouldComplete_WhenNothingIsToBeSaid() {
        ...
        do {
            try dialog.asynchronouslyWaitUntilCompleted()
            hasCompleted = true
        } catch _ {
            hasCompleted = false
        }
    ...
    func test_talk_ShouldDisposeInterruptingConversation_WhenFinished() {
        ...
        assert(dialog: ["Utterance 1", "Utterance 2", "Utterance 3"],
                ...
                whenInputIs: {
                    ...
                    case "Utterance 2":
                        self.async {
                            self.sendInput(NSError(domain: "Hello Error", code: 0, userInfo: nil)) // This error should not be received by subScript since it finished with "Utterance 3"
                        }
